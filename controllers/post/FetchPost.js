...
exports.fetchAllComment = async (req, res) => {
  ...
  try {
    ...
    const totalCount = await PostComment.countDocuments({
      post: req.params.post_id,
      parent: null,
    });
...
exports.fetchAllReaction = async (req, res) => {
  try {
    const reacts = await Post.findById(req.params.post_id)
      .populate("reactions")
      .select("reactions");
    return res.status(200).json({ reacts: reacts });
  } catch (err) {
    console.log(err);
    return res.status(500).json({ error: "Something went wrong" });
  }
...
exports.fetchTrendingPost = async (req, res) => {
  ...
  try {
    ...
    return res.status(200).json({ posts: posts, pagination: paginationData });
  } catch (err) {
    console.log(err);
    return res.status(500).json({ error: "Something went wrong" });
  }
