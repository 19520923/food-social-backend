...
exports.fetchAllChat = async (req, res) => {
  ...
  try {
    const chats = await Chat.find({
      $or: [{ user_1: req.userId }, { user_2: req.userId }],
    })
      .sort({ created_at: -1 })
      .limit(limit)
      .skip(page * limit)
    ...
    return res.status(200).json({ chats: chats, pagination: paginationData });
  } catch (err) {
    console.log(err);
...
exports.fetchAllMessage = async (req, res) => {
  ...
  try {
    ...
    res.status(200).json({ messages: messages, pagination: paginationData });
  } catch (err) {
    console.log(err);
    return res.status(500).json({ error: "Something went wrong" });
  }
